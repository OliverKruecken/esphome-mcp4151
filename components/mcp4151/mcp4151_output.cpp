...
void MCP4151Output::setup() {
  ...
  this->sdio_pin_->digital_write(false); // SDIO idle low
}
...
void MCP4151Output::set_wiper_(uint16_t value) {
  // 16-bit command: bits[15:10]=0 (address 0, opcode write), bit[9]=0 (simplified,
  // no status read), bits[8:0]=value. Because address=0 and opcode=write=0,
  // the command word equals the wiper value directly.
